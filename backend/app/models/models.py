...
class User(Base):
    ...
    # Relacionamentos
    created_tickets: Mapped[list["Ticket"]] = relationship(back_populates="creator", foreign_keys="Ticket.creator_id")
    assigned_tickets: Mapped[list["Ticket"]] = relationship(back_populates="assignee", foreign_keys="Ticket.assignee_id")
...
class Product(Base):
    ...
    # Relacionamentos
    equipments: Mapped[list["Equipment"]] = relationship(back_populates="product", cascade="all, delete-orphan")
...
class Ticket(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    protocol: Mapped[str] = mapped_column(String(20), unique=True, nullable=False, index=True)  # HS-2026-0001
    ...
    assignee_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), index=True)
    product_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("products.id"))
    equipment_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("equipments.id"))

    # SLA
    sla_config_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("sla_configs.id"))
    ...
    # Relacionamentos
    creator: Mapped["User"] = relationship(back_populates="created_tickets", foreign_keys=[creator_id])
    ...
    histories: Mapped[list["TicketHistory"]] = relationship(back_populates="ticket", cascade="all, delete-orphan")
    attachments: Mapped[list["Attachment"]] = relationship(back_populates="ticket", cascade="all, delete-orphan")
    chat_messages: Mapped[list["ChatMessage"]] = relationship(back_populates="ticket", cascade="all, delete-orphan")
    satisfaction_survey: Mapped["SatisfactionSurvey | None"] = relationship(back_populates="ticket", uselist=False)
...
class ChatMessage(Base):
    ...
    __table_args__ = (
        Index("ix_chat_messages_ticket_created", "ticket_id", "created_at"),
    )
...
class KBArticle(Base):
    ...
    status: Mapped[KBArticleStatus] = mapped_column(Enum(KBArticleStatus), default=KBArticleStatus.draft, index=True)
...
class SatisfactionSurvey(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    ticket_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("tickets.id", ondelete="CASCADE"), unique=True, index=True)
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
    ...
    __table_args__ = (
        Index("ix_satisfaction_surveys_rating", "rating"),
    )
