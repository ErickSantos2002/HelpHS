...
const RECOMMEND_LABELS: Record<number, string> = {
  ...
  7: "Bem provável",
  8: "Recomendaria",
  9: "Recomendaria com certeza",
  10: "Recomendaria a todos",
};
...
export default function TicketDetailPage() {
  ...
  async function handleToggleAi() {
    ...
    try {
      setTicket(await toggleTicketAi(ticket.id, !ticket.ai_enabled));
      // Sem "a Helô parou": ela é uma das IAs do chamado, e o mesmo
      // interruptor cala a classificação automática e a sugestão de resposta.
      toast.success(desligando ? "IA desligada neste chamado." : "IA religada neste chamado.");
    } catch (err) {
      toastApiError(err, "Não foi possível mudar a IA deste chamado.");
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_256px] gap-5 lg:flex-1 lg:min-h-0">
        ...
        <div className="flex flex-col lg:min-h-0 min-w-0">
          <div className="lg:flex-1 lg:min-h-0 lg:overflow-y-auto">
                  ...
                  <div className="shrink-0 flex flex-col gap-3 rounded-xl border border-border/40 bg-background-surface px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <p className="text-sm text-slate-400">
                      ...
                      {withinReopenWindow ? (
                        ...
                        <>
                          O prazo para reabrir terminou em{" "}
                          {reopenDeadline.toLocaleDateString("pt-BR")}. Se o problema voltar, abra
                          um novo chamado.
                        </>
                      )}
                    </p>
            ...
            {activeTab === "kb" && <KBSuggestionsPanel ticketId={ticket.id} />}
                      ...
                      {obsEdit ? (
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            <Button size="sm" onClick={handleSaveObs} loading={obsSaving}>
                              Salvar
                            </Button>
                          </div>
                        </div>
              ...
              {tagsEdit ? (
                <div className="space-y-3">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <Button size="sm" onClick={handleSaveTags} loading={tagsSaving}>
                      Salvar
                    </Button>
                  </div>
                </div>
      ...
      >
        <div className="space-y-4">
          ...
          <ModalFooter>
            ...
            <Button onClick={handleAddNote} loading={noteSaving} disabled={!newNoteContent.trim()}>
              Salvar nota
            </Button>
          </ModalFooter>
        </div>
      ...
      >
        ...
        <ModalFooter>
          ...
          {user?.role === "admin" ? (
            ...
            <Button onClick={() => handleAssign(user!.id)} loading={assignLoading}>
              Assumir ticket
            </Button>
          )}
        </ModalFooter>
